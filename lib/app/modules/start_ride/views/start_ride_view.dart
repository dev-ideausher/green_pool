        ...
        () => controller.isLoad.value
            ...
            : GoogleMap(
                initialCameraPosition: CameraPosition(
                  target: LatLng(controller.myRidesModel.value.riderBookingDetails?.origin?.coordinates?.last ?? 0.0,
                      controller.myRidesModel.value.riderBookingDetails?.origin?.coordinates?.first ?? 0.0),
                  zoom: 14,
                ),
                ...
                polylines: {Polyline(polylineId: const PolylineId('route'), visible: true, width: 4, color: ColorUtil.kSecondary01, points: controller.polylineCoordinates)},
              ),
      ),
      bottomSheet: Column(
        ...
        children: [
              ...
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index1) {
                    final rider = controller.myRidesModel.value.driverBookingDetails!.riders![index1];
                    return Container(
                      ...
                      child: ClipOval(
                        child: SizedBox.fromSize(
                          ...
                          child: !controller.myRidesModel.value.driverBookingDetails!.riders!.isEmpty
                              ...
                              : Image(
                                  image: NetworkImage("${rider?.profilePic?.url}"),
                                ),
                        ),
                      ),
          ...
          ListTile(
            leading: SizedBox(
                ...
                child: ClipRRect(
                    ...
                    child: CommonImageView(url: controller.myRidesModel.value.riderBookingDetails?.riderDetails?.profilePic?.url, height: 40, width: 40))),
            title: Text(controller.myRidesModel.value.riderBookingDetails?.riderDetails?.fullName ?? "", style: TextStyleUtil.k16Medium()),
            /* subtitle: Text(
            ...
            trailing: Row(
              ...
              children: [
                InkWell(
                  onTap: () async {
                    await launchUrl(Uri.parse("tel:${controller.myRidesModel.value.riderBookingDetails?.riderDetails?.phone}"));
                  },
          ...
          Padding(
            ...
            child: OriginToDestination(
                origin: "${controller.myRidesModel.value.riderBookingDetails?.origin?.name}",
                destination: "${controller.myRidesModel.value.riderBookingDetails?.destination?.name}"),
          ),
          Obx(
            () => GreenPoolButton(
              ...
              label: controller.isRideStarted.value
                  ? controller.myRidesModel.value.driverBookingDetails!.isStarted!
