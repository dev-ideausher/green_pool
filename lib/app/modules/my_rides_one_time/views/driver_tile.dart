    ...
    return GetBuilder<MyRidesOneTimeController>(builder: (controller) {
      return GestureDetector(
        ...
        child: Container(
          ...
          decoration:
              BoxDecoration(color: ColorUtil.kWhiteColor, borderRadius: BorderRadius.circular(8.kh), border: Border(bottom: BorderSide(color: ColorUtil.kNeutral7, width: 2.kh))),
          child: Column(
            ...
            children: [
              ListTile(
                ...
                leading: SizedBox(
                    height: 40.kh,
                    width: 40.kw,
                    child: ClipRRect(borderRadius: BorderRadius.circular(100.kh), child: CommonImageView(url: Get.find<HomeController>().userInfo.value.data?.profilePic?.url))),
                title: Text(Get.find<HomeController>().userInfo.value.data?.fullName ?? "", style: TextStyleUtil.k16Bold()),
                subtitle: Text(
                  // '07 November 2023',
                  myRidesModelData.date?.split('T')[0] ?? ((myRidesModelData.time ?? "") == "" ? "" : "${myRidesModelData.time}"),
                ...
                trailing: SizedBox(
                  ...
                  child: Align(
                    ...
                    child: ListView.builder(
                      shrinkWrap: true,
                      itemCount: (myRidesModelData.postsInfo?.length ?? 0) == 0 ? 4 : myRidesModelData.postsInfo?.length,
                        ...
                        return Container(
                          ...
                          child: ClipOval(
                            child: SizedBox.fromSize(
                              ...
                              child: (myRidesModelData.postsInfo?.length ?? 0) == 0
                                  ? Image.asset(
                                ImageConstant.pngEmptyPassenger,
                              )
                                  : CommonImageView(url: "${myRidesModelData.postsInfo?[index1]?.riderPostsDetails?[0]?.ridersDetails?[0]?.profilePic?.url}"),
                            ),
                          ),
             ...
             // Text(myRidesModelData.origin?.originDestinationFair??"0", style: TextStyleUtil.k12Regular(color: ColorUtil.kBlack03)),
              ...
              myRidesModelData.time == ""
                  ...
                  : Row(
                      children: [
                        ...
                        Text(
                          // '07 November 2023',
                          myRidesModelData.date?.split('T')[0] ?? "",
                          style: TextStyleUtil.k12Regular(color: ColorUtil.kBlack03),
                        ),
                      ],
              ...
              OriginToDestination(
                ...
                destination: myRidesModelData.destination?.name ?? Strings.destination,
              ).paddingOnly(bottom: 8.kh),
              Container(
                width: 100.w,
                height: 1.kh,
              ...
              Obx(
                () => myRidesModelData.isCancelled == true
                    ? Text(
                            ...
                            ? Row(
                                ...
                                children: [
                                  GreenPoolButton(
                                      ...
                                      padding: EdgeInsets.all(8.kh),
                                      fontSize: 14.kh,
                                      label: (myRidesModelData.isCompleted ??false)? Strings.rideCompleted :(myRidesModelData.postsInfo?.isEmpty?? false)
                                          ?Strings.viewDetails:Strings.startRide),
                                  GreenPoolButton(
                                    onPressed: () {
                                      controller.cancelRideAPI(myRidesModelData);
                                    },
